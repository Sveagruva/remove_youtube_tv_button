...
chrome.storage.sync.get(['enabled'], async result => {
    // enabled no longer disables the extension )) im lazy
    if(result.enabled !== false) {
        console.log('0.1.3 youtube remove button (relying on css to hide the button)');
        return;
    } else {
        console.log('0.1.2 youtube remove button');
        // 0.1.2 version of the extension
